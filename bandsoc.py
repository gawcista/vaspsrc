...
def correction():
	...
	band=float(getnum(subprocess.getoutput('grep E-fermi '+DIR+'bandsoc/OUTCAR'))[0])
	if FERMI == '0':
		E_fermi = 0.
		return -band
	else: 
...
def read_EIGENVAL():
	...
	fin = open(DIR+'bandsoc/EIGENVAL','r')
	lines = fin.readlines()
	fin.close()
	NK = int(getnum(lines[5])[1])
	NBAND = int(getnum(lines[5])[2])
	ISPIN =int(getnum(subprocess.getoutput('grep ISPIN '+DIR+'bandsoc/OUTCAR'))[0])
	os.system('grep -A '+str(NK)+' 2pi/SCALE '+DIR+'bandsoc/OUTCAR >'+DIR+'kpoints.temp')
...
def writePlot(y0,y1,color,inputfilename,outputfilename='output.eps',dir='',filename='plot_test'):
	...
	fout.write('set arrow from 0,0 to '+str(Kpoints[NK-1][3])+',0 nohead lt 0 lw 2\n')
	fout.write('plot ')
...
def dirGenerator():
	...
	for x in M:
		for y in O:
			for z in X:
				name = x+y+z
				if os.path.exists(name+'/bandsoc/EIGENVAL'):
					initialize()
					DIR = name+'/'
					outputfilename = 'pic/'+name+'-bandsoc'
					read_EIGENVAL()
...
def run():
	initialize()
	read_EIGENVAL()
	print(float(getnum(subprocess.getoutput('grep E-fermi '+DIR+'bandsoc/OUTCAR'))[0]))
	#plot(3.02527,20.03929,8,7,'','band.eps')
	plot(-15,20,8,7,DIR,'bandsoc0.eps')
	plot(-1,1,8,7,DIR,'bandsoc1.eps')
	plot(-0.005,0.005,8,7,DIR,'bandsoc2.eps')
	#dirGenerator()
run()
#print(Special)
#dirGenerator()
